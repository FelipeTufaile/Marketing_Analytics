...
columns <- c("transac", "rent", "vol", "tempo", "educ", "idade", "mulher1",
             "carro", "telefone1", "internet1", "computador1", "renda",
             "sobras", "cartcred", "cxelet", "chequeesp", "pagautom", 
             "financ", "finimob", "parcel", "fundoinv", "cdb", "previd", 
             "segurocarro", "segurocasa", "segurovida", "cluster3")

stdComp <- banco[, which(names(banco) %in% columns)] %>%
  mutate(cluster3 = as.character(cluster3)) %>% 
  group_by(cluster3) %>%
  summarise_all(.tbl=., .funs=sd) %>%
  bind_rows(., banco[, which(names(banco) %in% columns)] %>%
                 mutate(cluster3='All') %>%
                 group_by(cluster3) %>%
                 summarise_all(.tbl=., .funs=sd))
